...
function renderDetailedBlocks(result) {
  ...
  const sourceFilePatterns = [
    /\.(js|jsx|ts|tsx|vue|svelte)$/, // Source files
    /^[^.]*$/, // Files without extensions (e.g., Makefile)
  ];

  // Patterns to ignore
  const ignoredPatterns = [
    /node_modules/, // Node modules
    /\.(test|spec|stories|mock)\.[jt]sx?$/, // Test and story files
    /\.d\.ts$/, // TypeScript declaration files
    /\.(png|jpg|jpeg|gif|svg|ico|woff|woff2|ttf|eot)$/i, // Images and fonts
  ...
  if (result.files) {
    result.files.forEach((file) => {
      ...
      const isLargeFile = lineCount > 500; // Changed threshold to 500 lines
      ...
      lines.forEach((line) => {
        ...
        // Count if/else statements (but not else if as separate)
      ...
      // Calculate complexity score (more sophisticated formula)
      const complexity =
  ...
  if (codeHealthIssues.length > 0 || largeFiles.length > 0) {
    html += `
    <div style="border:1px solid #e5e7eb;border-radius:8px;padding:16px;margin-bottom:16px;">
      <h3 style="margin:0 0 12px 0;font-size:14px;font-weight:bold;display:flex;align-items:center;gap:8px;">
          ...
          ${codeHealthIssues
            .map((file) => {
              ...
              if (file.nestingLevel > 5)
                issues.push(`глибоке вкладення (${file.nestingLevel} рівнів)`);
              ...
              if (severityScore > 70) severityColor = "#ef4444"; // red
              else if (severityScore > 50) severityColor = "#f59e0b"; // amber
              else if (severityScore > 30) severityColor = "#3b82f6"; // blue
